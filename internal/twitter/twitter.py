    ...
    def extract_tweet_id_from_url(tweet_url: str) -> str:
        ...
        try:
            ...
            if '/status/' in tweet_url:
                # Standard format: https://x.com/username/status/123456
                tweet_id = tweet_url.split('/status/')[1].split('?')[0].split('/')[0]
            else:
                # If URL contains the ID directly
                tweet_id = tweet_url.split('s=')[1].split('&')[0]
            return tweet_id
        except Exception as e:
            raise Exception(f'Failed to extract tweet ID from URL: {str(e)}')

    async def reply_to_tweet(self, tweet_url: str, reply_text: str) -> str:
        """Reply to a specific tweet using its URL."""
        try:
            tweet_id = self.extract_tweet_id_from_url(tweet_url)
            return await self.post_tweet(reply_text, tweet_id=tweet_id)
        except Exception as e:
            raise Exception(f'Reply to tweet error: {str(e)}')
